...
def download_historical_models_data(output_dir, days_back=None, start_date=None, end_date=None, include_current=True):
    """
    ...
    Args:
        ...
        start_date: Start date (datetime object or None). If specified, only fetch commits on or after this date.
        end_date: End date (datetime object or None). If specified, only fetch commits on or before this date.
        include_current: If True, also download the current models.parquet file to the output_dir
    ...
    if start_date and end_date:
        logging.info(f"Filtering commits from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    elif days_back:
    ...
    try:
        ...
        # If start_date is specified, fetch all commits and filter later
        commits = _fetch_all_commits_with_pagination(
            repo_id=repo_id,
            repo_type="dataset",
            days_back=None if (start_date or end_date) else days_back
        )

        logging.info(f"Found {len(commits)} commits" + (f" from the past {days_back} days" if days_back and not (start_date or end_date) else ""))

        # Apply custom date range filter if specified
        if start_date or end_date:
            date_filtered_commits = []
            for commit in commits:
                ...
                if start_date and commit_date < start_date.replace(tzinfo=commit_date.tzinfo):
                    continue
                if end_date and commit_date > end_date.replace(hour=23, minute=59, second=59, tzinfo=commit_date.tzinfo):
                    continue
                date_filtered_commits.append(commit)

            logging.info(f"Filtered to {len(date_filtered_commits)} commits within date range")
        else:
        ...
        # Download current models.parquet if requested
        if include_current:
            logging.info("\nDownloading current models.parquet...")
            current_path = download_models_data(output_dir)
            downloaded_files.append(current_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Download HuggingFace models data from hub-stats dataset'
    )
    parser.add_argument(
        '--output-dir',
        '-o',
        ...
        help='Output directory for downloaded files (default: hf_scraper/data/raw)'
    )
    parser.add_argument(
        '--historical',
        action='store_true',
        help='Download historical versions to output-dir/historical/'
    )
    parser.add_argument(
        '--days-back',
        type=int,
        help='Number of days to look back in commit history (only with --historical)'
    )
    parser.add_argument(
        '--debug',
        action='store_true',
    ...
    try:
        if args.historical:
            logging.info("="*60)
            logging.info("HISTORICAL MODE: Downloading historical models data")
            logging.info("="*60)
            ...
            logging.info("="*60)
            logging.info(f"Downloaded {len(downloaded_files)} files successfully")
            logging.info("="*60)
        else:
            logging.info("="*60)
            logging.info("STANDARD MODE: Downloading current models data")
            logging.info("="*60)

            output_path = download_models_data(output_dir)
